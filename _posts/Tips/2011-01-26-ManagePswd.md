...
Google OTP 재발급 토큰, 암호화 화폐 keystore 파일 등과 같은  
보관해야하지만 외울 수 없는 긴 문자열에 대해서 사용하시면 됩니다.  

외워야하는 문자열이 100개일지라도 1개의 암호화키만 외우면 됩니다.  
암호화키는 반드시 12글자 이상을 이용하세요. 대소문자 섞고 특수문자 넣으면 더 좋습니다.  
...
### 암호화 과정
  ...
  * ```cu2ckr7ex778d3l4``` 와 같은 OTP 재발급 토큰 값과 암호화키(```암호화키는 길수록 좋습니다```)를 입력 후 ```암호화``` 버튼을 클릭합니다.  
...
### 복호화 과정 
  ...
  * 보관하고 계시던 암호화 된 문자열(```U2FsdGVkX1+k4baEw3OodvGSGfU55zX+ImYE8HlZ3DJPTfdrHAP7TY3HaUlHby72```)와 기억하고 계시던 암호화키(```암호화키는 길수록 좋습니다```)를 입력 후 ```복호화``` 버튼을클릭합니다.  

  * 복호화 된 문자열(```cu2ckr7ex778d3l4```)를 복사하셔서 사용하시면 됩니다.  

### 주의사항 

다시한번 말씀드리지만 암호화키는 반드시 12글자 이상을 사용하세요.  

해커가 1초에 3,000,000번의 시도를 할 수 있다고 치면,  
소문자로만 이뤄진 8글자의 암호화키는 1일내에 풀리게 됩니다.  

12글자면 1022년이 걸립니다.  
물론 해커가 사용하는 장비를 1000대로 늘리면 1년만에 풀리게 되겠죠.  
근데 한 개인의 패스워드를 풀기위해 그렇게까진 하진 않을 것 같습니다.  

근데 한번 더 생각해봅시다. 이왕 길게가는거 20글자 이상은 어떤가요?  
20글자는 5,600,000,000,000,000년 걸리게 됩니다.  
양자컴퓨터가 이용되기 전까지는 해커가 돈을 쏟아부어도 풀 방법이 없습니다.  

사실 20글자는 굉장히 짧습니다. 단어가 아닌 문장을 이용한다면 외우기도 쉽습니다.  
...
사용하실 때는 압축을 푸시면 됩니다. (이 또한 암호화키는 12글자 이상을 사용하세요.)  

물론 암호는 꼭 기억해두시고요.  

비트코인, 이더리움 같은 암호화화폐의 keystore 파일 보관할 때 유용하겠죠?  
암호화화폐 keystore 파일은 재발급이 안되니 보관을 잘하세요.  
