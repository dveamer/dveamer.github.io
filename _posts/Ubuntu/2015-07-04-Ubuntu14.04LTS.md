...
왼쪽 아래의 + 버튼을 이용해서 한국어, 영문 두개를 세팅합니다.  
...
## 한글 관련 버그 한방해결 Script

아래 버그 내용은 제가 우분투를 처음 설치했을 때 존재하던 것이고  
현재 최신버전으로 업그레이드하면 모두 해결됩니다.  
...
해당 설정은 추천하지 않습니다.  
최초에 세팅하고 예상대로 돌아가는 것을 확인했지만  
사용하다보니 한영키를 눌러도 아무런 반응이 없는 상황에 놓이게 됩니다.  

결국 Shift + Space 로 한/영 전환하도록 세팅해서 사용중입니다.  
Shift + Space 세팅이라면 굳이 아래의 donf-editor 를 설치할 필요가 없습니다.  
Text Entry 설정에서 세팅이 가능합니다.  

<br>

```dconf-editor``` 를 설치 후 실행합니다.   
...
좌측 트리에서 ```org > gnome > desktop > wm > keybindings``` 를 찾습니다.  
```switch-input-source``` 항목의 값을 ```Hangul``` 으로 변경합니다.  

![Hangul-English Key](/images/post_img/Ubuntu/ubuntu_hangul.png)

테스트해서 안되신다면 재부팅 해보세요.

## The shortcut to HUD
 * '한/영' 키 입력시 HUD(Heads UP Display) 창으로 전환되는 문제 발생시 shortcut 수정
