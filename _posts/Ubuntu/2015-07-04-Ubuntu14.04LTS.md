...
14.10 버전은 안정화가 안됐다라는 글과 15.x 버전은 아직 고생길일 것 같다는 느낌이 들어서  
14.04 버전의 최상위 버전으로 업그레이드 합니다.
...
15.x 버전으로 업그레이드 하시려면 ```any newer version``` 으로 변경하시고 알림을 기다리시면 됩니다.  
다만, 15.x 버전으로 바꾸신다면 한글 설정에 대해서는 다시 검색해보셔야 할 것 같습니다.  

## Language Download

```System Setting > Language Support``` 를 실행합니다. 자동으로 다운로드 합니다.  
한글이 다운받아졌는지 체크해보세요.  
