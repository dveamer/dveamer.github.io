...
```Java
    Logger logger = Logger.getLogger("package1");

    String msg = "원본 메시지";
    logger.debug(msg); // String 은 immutable object

    MessageObject msgObject = new MessageObject(); // 임의의 object
    msgObject.setMsg(msg);
    logger.debug(msgObject); /* "변경될지도 모른다." 라고 출력 될 수도 있습니다.
    ...
    */
    ...
    logger.debug(msgObject.toString()); /* "변경될리 없다." 라고 출력됩니다. 
...
```Java
    ...
    */
    ...
    logger.debug(msg); // "다른 쓰레드 메시지" 라고 출력 될 것입니다.
```
