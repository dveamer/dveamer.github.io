...
### 그룹사 홈 상품 E-Commerce 구축 프로젝트 (2020.08~2021.06)
...
50명 규모의 개발자가 투입된 신규 구축 프로젝트 입니다. Application Architect 역할을 수행으며 MSA, Cloud, Kubernetes 등이 익숙하지 않은 업무 전문 개발자분들도 개발할 수 있도록 구성하는데 많은 노력을 기울였습니다. 특히, 고객사의 클라우드 선정이 GCP->AWS->GCP 로 크게 변경되었지만 이를 예상하고 Kubernetes를 이용했고 CSP(Cloud Service Provider)에 종속되는 라이브러리, API 호출은 모두 래핑하여 제공하여 개발자들은 CSP 변경에도 영향을 받지 않도록 진행했습니다.  
  ...
  * 파일 메타정보 관리 서비스 설계&구축
    ...
    - 파일 메타 정보 API 제공
    - 파일 후속 작업 처리 : 이미지 파일 썸네일 처리, HTML 파일 XSS 제거, 동영상 파일 비디오 트랜스코딩 (설계만) 
