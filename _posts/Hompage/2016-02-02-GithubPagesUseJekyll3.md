...
>Jekyll 3.0 기본세팅에서는 작성시각이 UTC +00 기준으로 설정입니다.
2016.02.02 23:00 로 입력했을 경우,  
영국시각으로 2016.02.02 23:00 이 되고  
우리나라 시각으로는 02.03 08:00 가 됩니다.

>config에 아래와 같이 입력을 하면 우리나라 시각을 이용해서 편하게 이용할 수 있습니다.
>timezone: '+0900'  

라고 이해했는데 홈페이지에 출력되는 모양이 예상과 다르다.  
지금은 잘 모르겠다. 나중에 좀 더 맑은 정신으로 살펴봐야할 듯 합니다.

하지만 제 홈페이지는 등록시각은 별 의미가 없고 날짜만 출력되면 되기 때문에  
등록시각을 모두 12:00으로 맞출 예정입니다. 그러면 어쨋든 예상한 날짜로 출력됩니다.  
