...
더 구체적으로 설명하자면 Markdown 형식으로 작성한 문서를 Jekyll 가 정적인 HTML 형태로 변환해줍니다.
그리고 그 변환된 HTML을 Github-Page에 올려서 지금 이렇게 볼 수 있는 것입니다.

한번 더 깊게 설명하자면, Github-Page 에다가 Markdown 형식의 문서를 올리면 Github가 Jekyll를 이용해서 정적인 HTML로 변환해주고 우리는 Github-Page에서 브라우저로 볼 수 있게 됩니다.
...
# Thr list of contents
  * 모든 post 상단에 목차를 추가한다.
  * markdown 이 제공하는 방법이 있긴한데 markdown 은 변경할 때마다 모든 글을 수정해야하는 문제가 있다. 그래서 script 로 진행하는 방법을 선택한다.
...
<script type="text/javascript">
 $(document).ready(function(){
    $("h1,h2,h3,h4,h5,h6").not(".subtitle").not(".post-title").each(function(i,item){
